...
public class SuffixTree<T extends Node> implements Iterable<T>
{
    ...
    public Node build(ISequence input)
    {
        ...
        final int maxIndex = this.getInput().size();
    ...
    private Node getMatchingNode(ISequence seq)
    {
        ...
        while (index < seq.size())
        {
            ...
            for (int i = e.getStartIndex(); i <= endIndex; i++, index++)
            {
                ...
                if (getInput().objectAt(i) != seq.objectAt(index))
                {
                    /* Not found. */
                    return null;
                }
    ...
    final Suffix insertPrefix(Suffix active, int lastElementIndex)
    {
        ...
        while (true)
        {
            ...
            if (active.isExplicit())
            {
                 ...
                 */
                edge = active.originNode.getEdge(getInput().objectAt(lastElementIndex));
            ...
            else
            {
                ...
                edge = active.originNode
                    .getEdge(getInput().objectAt(active.firstElementIndex));

                int span = active.lastElementIndex - active.firstElementIndex;

                if (getInput().objectAt(edge.firstElementIndex + span + 1) == getInput()
             ...
             */
            parent_node.createChildNode(lastElementIndex, getInput().size() - 1);
